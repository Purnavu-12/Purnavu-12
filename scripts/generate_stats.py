...
Env:
  ...
  GH_LOGIN      user to summarise (default: Purnavu-12)
  OUT_DIR       where to write (default: repository root)
  REFRESH_PORTRAIT  if set, regenerate the portrait from avatar
"""
import base64
import functools
import json
...
#  * the contribution window, to whole UTC days
...
# The portrait's ink is the data ink
...
def face(filename, weight):
    ...
    Falls back to an empty string (no custom font) if the file is missing,
    so the SVGs still render with the system monospace stack."""
...
def draw_connect():
    """The Connect heading with thanks text."""
    H = 70
    p = [head(WIDTH, H, font=font_head())]
    # Heading text + hairline (same as draw_heading)
    text_end = len("connect") * 16 * 0.6 + 18
    p.append(label(0, 18, "connect", 16, "e-f", extra=' font-weight="600"'))
    p.append(f'<line x1="{text_end:.0f}" y1="12.5" x2="{WIDTH}" y2="12.5" '
             f'class="u-s" stroke-width="1"/>')
    # Thanks text
    p.append(label(WIDTH // 2, 48,
                    "Thanks for stopping by! Feel free to explore, star ⭐, and fork.",
                    11, "m-f", "middle"))
    p.append("</svg>")
    return "".join(p)


def draw_badge(label_text):
    """A standalone badge SVG (180×28) with rounded rectangle and label."""
    W = 180
    H = 28
    p = [head(W, H, font=font_head())]
    p.append(f'<rect x="0" y="0" width="{W}" height="{H}" rx="6" '
             f'class="u-s" fill="none" stroke-width="1"/>')
    p.append(label(W // 2, 19, label_text, 13, "e-f", "middle",
...
def regenerate_portrait(token, login):
    """Download avatar, create ASCII portrait, embed font."""
    # Get avatar URL
    avatar_url = f"https://api.github.com/users/{login}"
    req = urllib.request.Request(
        avatar_url,
        headers={"User-Agent": f"{login}-profile"})
    with urllib.request.urlopen(req, timeout=30) as r:
        user_data = json.load(r)
    
    avatar_url = user_data.get("avatar_url", "").replace("?s=", "?s=400")
    
    # Download avatar
    avatar_path = os.path.join(os.path.dirname(__file__), "avatar.png")
    with urllib.request.urlopen(avatar_url, timeout=30) as r:
        with open(avatar_path, "wb") as f:
            f.write(r.read())
    
    print(f"Downloaded avatar from {avatar_url}")
    
    # Run make_portrait.py
    import subprocess
    result = subprocess.run(
        [sys.executable, "make_portrait.py", avatar_path, "--cols", "90", "--preview"],
    ...
    # Embed font
    result = subprocess.run(
        [sys.executable, "embed_portrait_font.py"],
        cwd=os.path.dirname(__file__),
        capture_output=True,
        text=True
    )
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "Purnavu-12")
    out_dir = os.environ.get("OUT_DIR", ".")
    refresh_portrait = os.environ.get("REFRESH_PORTRAIT", "")

    if refresh_portrait:
        print("Regenerating portrait from avatar...")
        regenerate_portrait(token, login)

    s = summarise(fetch(login, token))
    files = {}
    for word in ("about", "stack", "projects"):
        files[f"hd-{word.replace(' ', '-')}.svg"] = draw_heading(word)
    files["hd-connect.svg"] = draw_connect()
    for label_name in ("github", "linkedin", "leetcode"):
        files[f"badge-{label_name}.svg"] = draw_badge(label_name.capitalize())
